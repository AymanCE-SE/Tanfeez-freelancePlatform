...
export default function PricingBox({ price, serviceId, isOwnService, alreadyOrdered, onOrderSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Order Sent',
        text: 'Your order has been sent to the freelancer.',
        timer: 2000,
      ...
      Swal.fire({
        icon: 'error',
        title: 'Order Failed',
        text: error?.response?.data?.detail || error?.response?.data?.[0] || 'Something went wrong. Please try again.',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <>
      <Card className="shadow-sm p-4 border-0 mb-3" style={{ background: '#f8fffa' }}>
        ...
        {/* A freelancer viewing their own service, or someone not logged in
            as a client, has no business ordering it — hide the button
            entirely rather than let them hit a 403 after clicking. */}
          {isOwnService ? (
          ...
          user?.user_type === 'client' && (
            <Button variant="success" size="lg" className="w-100 fw-semibold" disabled={alreadyOrdered} onClick={() => setShowModal(true)}>
              {alreadyOrdered ? 'Order Already Sent' : `Continue ($${price})`}
            </Button>
          )
        )}
